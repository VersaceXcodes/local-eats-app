...
describe('Authentication Tests', () => {
  ...
  describe('POST /api/auth/login', () => {
    beforeEach(async () => {
      ...
      await pool.query(
        `INSERT INTO user_statistics (stat_id, user_id, total_reviews_written, 
    ...
    it('should handle remember_me flag', async () => {
      ...
      const decoded = jwt.decode(response.body.auth_token) as any;
  ...
  describe('POST /api/auth/password-reset/request', () => {
    ...
    it('should send password reset request', async () => {
      ...
      const tokenResult = await pool.query(
        'SELECT * FROM password_reset_tokens WHERE user_id IN (SELECT user_id FROM users WHERE email = $1)',
        ['reset@example.com']
      );
  ...
  describe('POST /api/auth/password-reset/complete', () => {
    ...
    beforeEach(async () => {
      ...
      await pool.query(
        `INSERT INTO password_reset_tokens (token_id, user_id, reset_token, expires_at, is_used, created_at) 
        VALUES ($1, $2, $3, $4, $5, $6)`,
        ['token_test_001', userId, resetToken, expiresAt, false, new Date().toISOString()]
      );
    });
    ...
    it('should reset password with valid token', async () => {
      ...
      const tokenResult = await pool.query(
        'SELECT is_used FROM password_reset_tokens WHERE reset_token = $1',
        [resetToken]
      );
    ...
    it('should reject expired token', async () => {
      ...
      await pool.query(
        `INSERT INTO password_reset_tokens (token_id, user_id, reset_token, expires_at, is_used, created_at) 
        VALUES ($1, $2, $3, $4, $5, $6)`,
        ['token_expired', userId, expiredToken, expiredDate, false, new Date().toISOString()]
      );
    ...
    it('should reject already used token', async () => {
      // Mark token as used
      await pool.query(
        'UPDATE password_reset_tokens SET is_used = true WHERE reset_token = $1',
        [resetToken]
      );
...
describe('Restaurant Tests', () => {
  ...
  beforeEach(async () => {
    ...
    const result = await pool.query(
      `INSERT INTO restaurants (restaurant_id, restaurant_name, description, cuisine_types, 
  ...
  describe('GET /api/restaurants', () => {
    ...
    it('should sort by rating', async () => {
      ...
      if (response.body.restaurants.length > 1) {
        const ratings = response.body.restaurants.map((r: any) => r.average_rating);
  ...
  describe('GET /api/restaurants/:restaurant_id/hours', () => {
    beforeEach(async () => {
      ...
      for (let day = 0; day < 7; day++) {
        await pool.query(
          `INSERT INTO restaurant_hours (hours_id, restaurant_id, day_of_week, open_time, close_time, is_closed, created_at, updated_at) 
  ...
  describe('GET /api/restaurants/:restaurant_id/menu', () => {
    ...
    beforeEach(async () => {
      ...
      const catResult = await pool.query(
        `INSERT INTO menu_categories (category_id, restaurant_id, category_name, display_order, created_at, updated_at) 
        VALUES ($1, $2, $3, $4, $5, $6) RETURNING category_id`,
        ['cat_test_001', restaurantId, 'Appetizers', 0, new Date().toISOString(), new Date().toISOString()]
      );
      categoryId = catResult.rows[0].category_id;

      // Create menu item
      const itemResult = await pool.query(
        `INSERT INTO menu_items (menu_item_id, restaurant_id, category_id, item_name, description, 
  ...
  describe('GET /api/restaurants/:restaurant_id/menu/items/:menu_item_id', () => {
    ...
    beforeEach(async () => {
      const catResult = await pool.query(
        `INSERT INTO menu_categories (category_id, restaurant_id, category_name, display_order, created_at, updated_at) 
        VALUES ($1, $2, $3, $4, $5, $6) RETURNING category_id`,
        ['cat_test_002', restaurantId, 'Mains', 0, new Date().toISOString(), new Date().toISOString()]
      );
      categoryId = catResult.rows[0].category_id;
      ...
      // Add sizes
      await pool.query(
        `INSERT INTO menu_item_sizes (size_id, menu_item_id, size_name, price_adjustment, display_order, created_at) 
        VALUES ($1, $2, $3, $4, $5, $6)`,
        ['size_test_001', menuItemId, 'Small', 0, 0, new Date().toISOString()]
      );
      await pool.query(
        `INSERT INTO menu_item_sizes (size_id, menu_item_id, size_name, price_adjustment, display_order, created_at) 
        VALUES ($1, $2, $3, $4, $5, $6)`,
        ['size_test_002', menuItemId, 'Large', 5.00, 1, new Date().toISOString()]
      );

      // Add addons
      await pool.query(
        `INSERT INTO menu_item_addons (addon_id, menu_item_id, addon_name, addon_price, display_order, created_at) 
        VALUES ($1, $2, $3, $4, $5, $6)`,
        ['addon_test_001', menuItemId, 'Extra Cheese', 2.00, 0, new Date().toISOString()]
      );
    });
...
describe('Favorites Tests', () => {
  ...
  describe('POST /api/favorites', () => {
    it('should add restaurant to favorites', async () => {
      ...
      const favResult = await pool.query(
        'SELECT * FROM favorites WHERE user_id = $1 AND restaurant_id = $2',
        [testUserId, restaurantId]
      );
      expect(favResult.rows.length).toBe(1);

      // Verify statistics updated
      const statsResult = await pool.query(
        'SELECT total_favorites_saved FROM user_statistics WHERE user_id = $1',
        [testUserId]
      );
  ...
  describe('GET /api/favorites', () => {
    beforeEach(async () => {
      // Add favorite
      await pool.query(
        `INSERT INTO favorites (favorite_id, user_id, restaurant_id, created_at) 
        VALUES ($1, $2, $3, $4)`,
        ['fav_test_001', testUserId, restaurantId, new Date().toISOString()]
      );
    });
  ...
  describe('DELETE /api/favorites/:restaurant_id', () => {
    beforeEach(async () => {
      await pool.query(
        `INSERT INTO favorites (favorite_id, user_id, restaurant_id, created_at) 
        VALUES ($1, $2, $3, $4)`,
        ['fav_test_002', testUserId, restaurantId, new Date().toISOString()]
      );
      await pool.query(
        'UPDATE user_statistics SET total_favorites_saved = 1 WHERE user_id = $1',
        [testUserId]
      );
    });
...
describe('Cart Tests', () => {
  ...
  beforeEach(async () => {
    ...
    const catResult = await pool.query(
      `INSERT INTO menu_categories (category_id, restaurant_id, category_name, display_order, created_at, updated_at) 
      VALUES ($1, $2, $3, $4, $5, $6) RETURNING category_id`,
      ['cat_test_cart', restaurantId, 'Pizza', 0, new Date().toISOString(), new Date().toISOString()]
    );
...
describe('Order Tests', () => {
  ...
  beforeEach(async () => {
    ...
    const catResult = await pool.query(
      `INSERT INTO menu_categories (category_id, restaurant_id, category_name, display_order, created_at, updated_at) 
      VALUES ($1, $2, $3, $4, $5, $6) RETURNING category_id`,
      ['cat_test_order', restaurantId, 'Pizza', 0, new Date().toISOString(), new Date().toISOString()]
    );
    ...
    const pmResult = await pool.query(
      `INSERT INTO saved_payment_methods (payment_method_id, user_id, payment_label, card_type, 
  ...
  describe('POST /api/orders', () => {
    it('should create delivery order successfully', async () => {
      ...
      // Verify statistics updated
      const statsResult = await pool.query(
        'SELECT total_orders_placed FROM user_statistics WHERE user_id = $1',
        [testUserId]
      );
  ...
  describe('DELETE /api/orders/:order_id', () => {
    ...
    it('should cancel order when in order_received status', async () => {
      ...
      const orderResult = await pool.query(
        'SELECT order_status, cancelled_at, cancellation_reason FROM orders WHERE order_id = $1',
        [orderId]
      );
...
describe('Review Tests', () => {
  ...
  beforeEach(async () => {
    ...
    // Create mock order
    const orderResult = await pool.query(
      `INSERT INTO orders (order_id, user_id, restaurant_id, order_type, order_status, 
  ...
  describe('POST /api/reviews', () => {
    it('should create review successfully', async () => {
      ...
      const restResult = await pool.query(
        'SELECT average_rating, total_review_count FROM restaurants WHERE restaurant_id = $1',
        [restaurantId]
      );
  ...
  describe('DELETE /api/reviews/:review_id', () => {
    ...
    it('should delete review successfully', async () => {
      ...
      const restResult = await pool.query(
        'SELECT total_review_count FROM restaurants WHERE restaurant_id = $1',
        [restaurantId]
      );
  ...
  describe('GET /api/restaurants/:restaurant_id/reviews', () => {
    beforeEach(async () => {
      ...
      await pool.query(
        `INSERT INTO reviews (review_id, user_id, restaurant_id, star_rating, review_text, 
  ...
  describe('POST /api/reviews/:review_id/helpful', () => {
    ...
    it('should mark review as helpful', async () => {
      ...
      const markResult = await pool.query(
        'SELECT * FROM review_helpful_marks WHERE review_id = $1 AND user_id = $2',
        [reviewId, testUserId]
      );
      ...
      const reviewResult = await pool.query(
        'SELECT helpful_count FROM reviews WHERE review_id = $1',
        [reviewId]
      );
  ...
  describe('POST /api/reviews/:review_id/report', () => {
    ...
    it('should report review successfully', async () => {
      ...
      const reportResult = await pool.query(
        'SELECT * FROM review_reports WHERE review_id = $1',
        [reviewId]
      );
...
describe('Discount Tests', () => {
  ...
  beforeEach(async () => {
    ...
    const discResult = await pool.query(
      `INSERT INTO discounts (discount_id, restaurant_id, discount_type, discount_value, 
  ...
  describe('POST /api/discounts/:discount_id/redeem', () => {
    it('should redeem discount successfully', async () => {
      ...
      const redemptionResult = await pool.query(
        'SELECT * FROM discount_redemptions WHERE discount_id = $1 AND user_id = $2',
        [discountId, testUserId]
      );
      expect(redemptionResult.rows.length).toBe(1);

      // Verify redemption count incremented
      const discResult = await pool.query(
        'SELECT current_redemption_count FROM discounts WHERE discount_id = $1',
        [discountId]
      );
    ...
    it('should enforce max redemptions per user', async () => {
      ...
      for (let i = 0; i < 3; i++) {
        await pool.query(
          `INSERT INTO discount_redemptions (redemption_id, discount_id, user_id, redemption_method, 
            discount_amount_applied, redeemed_at) 
          VALUES ($1, $2, $3, $4, $5, $6)`,
          [`redeem_test_${i}`, discountId, testUserId, 'coupon_code', 5.00, new Date().toISOString()]
        );
      }
...
describe('Discount Cart Application Tests', () => {
  ...
  beforeEach(async () => {
    ...
    const catResult = await pool.query(
      `INSERT INTO menu_categories (category_id, restaurant_id, category_name, display_order, created_at, updated_at) 
      VALUES ($1, $2, $3, $4, $5, $6) RETURNING category_id`,
      ['cat_cartdisc', restaurantId, 'Main', 0, new Date().toISOString(), new Date().toISOString()]
    );
...
describe('Badge Tests', () => {
  ...
  beforeEach(async () => {
    ...
    const badgeResult = await pool.query(
      `INSERT INTO badges (badge_id, badge_name, badge_description, badge_icon_url, 
...
describe('Notification Tests', () => {
  ...
  describe('PATCH /api/notifications/:notification_id/read', () => {
    it('should mark notification as read', async () => {
      ...
      // Verify in database
      const notifResult = await pool.query(
        'SELECT is_read, read_at FROM notifications WHERE notification_id = $1',
        [notificationId]
      );
    ...
    beforeEach(async () => {
      ...
      await pool.query(
        `INSERT INTO notifications (notification_id, user_id, notification_type, message, 
          is_read, created_at) 
        VALUES ($1, $2, $3, $4, $5, $6)`,
        ['notif_test_002', testUserId, 'promotion', 'New deal!', false, new Date().toISOString()]
      );
    });
  ...
  describe('POST /api/addresses', () => {
    ...
    it('should unset other defaults when creating default address', async () => {
      ...
      const addressResult = await pool.query(
        'SELECT COUNT(*) as count FROM saved_addresses WHERE user_id = $1 AND is_default = true',
        [testUserId]
      );
  ...
  describe('GET /api/addresses', () => {
    beforeEach(async () => {
      const addrResult = await pool.query(
        `INSERT INTO saved_addresses (address_id, user_id, address_label, street_address, 
...
describe('Payment Method Tests', () => {
  ...
  describe('GET /api/payment-methods', () => {
    beforeEach(async () => {
      await pool.query(
        `INSERT INTO saved_payment_methods (payment_method_id, user_id, payment_label, 
...
describe('Search Tests', () => {
  ...
  describe('GET /api/search/recent', () => {
    beforeEach(async () => {
      await pool.query(
        `INSERT INTO search_history (search_id, user_id, search_query, search_type, created_at) 
        VALUES ($1, $2, $3, $4, $5)`,
        ['search_test_001', testUserId, 'italian', 'cuisine', new Date().toISOString()]
      );
    });
  ...
  describe('DELETE /api/search/recent', () => {
    beforeEach(async () => {
      await pool.query(
        `INSERT INTO search_history (search_id, user_id, search_query, search_type, created_at) 
        VALUES ($1, $2, $3, $4, $5)`,
        ['search_test_del', testUserId, 'pizza', 'general', new Date().toISOString()]
      );
    });
...
describe('Weekly Picks Tests', () => {
  ...
  beforeEach(async () => {
    ...
    await pool.query(
      `INSERT INTO weekly_local_picks (pick_id, restaurant_id, week_start_date, week_end_date, 
  ...
  describe('POST /api/recommendations/dismiss', () => {
    ...
    it('should dismiss recommendation', async () => {
      ...
      const dismissResult = await pool.query(
        'SELECT * FROM dismissed_recommendations WHERE user_id = $1 AND restaurant_id = $2',
        [testUserId, restaurantId]
      );
...
describe('Verification Tests', () => {
  ...
  describe('POST /api/verifications', () => {
    it('should create order-based verification', async () => {
      ...
      // Verify statistics updated
      const statsResult = await pool.query(
        'SELECT total_restaurants_visited FROM user_statistics WHERE user_id = $1',
        [testUserId]
      );
